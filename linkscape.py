...
'''Python bindings for making linkscape requests'''
...
class linkscapeException(Exception):
...
class UMCols:
    ...
    # The number of links (juice-passing or not, internal or external) to the
    # url
    links                        = 2048
    # The number of subdomains with any pages linking to the subdomain of the
    # url
    ...
    # will provide both the pretty 10-point score (ptrp) and the raw score
    # (ptrr)
    rootDomainMozTrust           = 524288
    # The portion of the url's mozRank coming from external links.  Requesting
    # this metric will provide both the pretty 10-point score (uemrp) and the
    # raw score (uemrr)
    ...
    # metric will provide both the pretty 10-point score (fjp) and the raw
    # score (fjr)
    subdomainDomainJuice         = 8388608
    # The mozRank of all pages on the root domain combined.  Requesting this
    # metric will provide both the pretty 10-point score (pjp) and the raw
    # score (pjr)
    ...
    # A score out of 100-points representing the likelihood for arbitrary
    # content to rank on this page
    pageAuthority                = 34359738368
    # A score out of 100-points representing the likelihood for arbitrary
    # content to rank on this dom
    domainAuthority              = 68719476736

    # This is the set of all free fields
    freeCols = (title | url | externalLinks | links | mozRank |
        subdomainMozRank | httpStatusCode | pageAuthority | domainAuthority)

    paidCols = (title | url | subdomain | rootDomain | externalLinks |
        subdomainExternalLinks | rootDomainExternalLinks | juicePassingLinks |
        subdomainsLinking | rootDomainsLinking | links |
        subdomainSubdomainsLinking | rootDomainRootDomainsLinking |
        mozRank | subdomainMozRank | rootDomainMozRank |
        mozTrust | subdomainMozTrust | rootDomainMozTrust | externalMozRank |
        subdomainExternalDomainJuice | rootDomainExternalDomainJuice |
        subdomainDomainJuice | rootDomainDomainJuice | httpStatusCode |
        linksToSubdomain | linksToRootDomain | rootDomainsLinkingToSubdomain |
        pageAuthority | domainAuthority)
...
class ATCols:
    ...
    # The number of subdomains on the same root domain with at least one link
    ...
    # The number of external subdomains with at least one link with this term
    # or phrase
    externalSubdomainsLinking  = 64
    # The number of (external) root domains with at least one link with this
    # term or phrase
    externalRootDomainsLinking = 128
    # The amount of mozRank passed over all internal links with this term or
    # phrase (on the 10 point scale)
    internalMozRankPassed      = 256
    # The amount of mozRank passed over all external links with this term or
    # phrase (on the 10 point scale)
    externalMozRankPassed      = 512
    # Currently only "1" is used to indicate the term or phrase is found in an
    # image link
    flags                      = 1024
    ...
    # These are all the paid columns
    paidCols = freeCols
...
class client:
    ...
    def signature(self, expires):
        ...
        return base64.b64encode(
            hmac.new(self.secret_key, tosign, hashlib.sha1).digest())
    ...
    def query(self, method, data=None, **params):
        ...
        try:
            ...
            if exc.code == 401:
                raise linkscapeException(exc.read())
            else:
